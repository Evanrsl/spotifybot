...
class SpotifyAPI(object):
    ...
    def get_user_playlist(self, _id):
        lookup_id = f"{_id}/playlists"
        return self.get_resource(lookup_id, resource_type='users')

    def get_playlist_items(self, _id):
        lookup_id = f"{_id}/tracks"
        return self.get_resource(lookup_id, resource_type='playlists')
    ...
    def create_playlist(self, _id, access_token, name="new playlist", description="New playlist"):
    ...
    def add_playlist_item(self, _id, uri, access_token):
        endpoint = f"https://api.spotify.com/v1/playlists/{_id}/tracks"
        request_body = json.dumps({"uris":[uri]})
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {access_token}"
        }
